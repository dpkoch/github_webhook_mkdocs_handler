...
def mkdocs_job(repository, branch, output_path):
    ...
    result = subprocess.run(
        ['git', 'clone', '--recursive', '-b', branch, 'https://github.com/{}.git'.format(repository)],
    ...
    if os.listdir(output_path):
        ...
        result = subprocess.run('rm -r {}/*'.format(output_path), shell=True, stdout=stdoutfile, stderr=subprocess.STDOUT)
        if not result.returncode == 0:
